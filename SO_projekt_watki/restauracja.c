...
static pthread_t zombie_thread;
static volatile sig_atomic_t zombie_thread_stop = 0;
static int zombie_thread_started = 0;
...
static void *zombie_reaper_thread(void *arg)
{
    (void)arg;
    int status = 0;
    while (!zombie_thread_stop)
    {
        zbierz_zombie_nieblokujaco(&status);

#ifdef TEST_NO_SLEEP
        // W testach bez sleep() nie chcemy busy-loopa.
        sched_yield();
#else
        struct timespec ts;
        ts.tv_sec = 0;
        ts.tv_nsec = 100L * 1000L * 1000L; // 100ms
...
static pid_t uruchom_potomka_exec(const char *file, const char *argv0) // uruchamia proces potomny przez fork()+exec()
{
    ...
    if (pid == 0)
    {
        ...
        // Uwaga: proces jest wielowątkowy (mamy wątek reapera zombie),
        // więc w dziecku po fork() nie wołamy nie-async-signal-safe funkcji.
        _exit(127);
    }
    if (pid < 0)
    {
...
static int awaryjne_zamkniecie_fork(void)
{
...
int main(void)
{
    ...
    *pid_kierownik_shm = pid_kierownik;
    ...
    // Osobny wątek zbierający zombie (nieblokująco) w tle.
    zombie_thread_stop = 0;
    if (pthread_create(&zombie_thread, NULL, zombie_reaper_thread, NULL) == 0)
        zombie_thread_started = 1;
    ...
    *kolej_podsumowania = 1;
    ...
    // Zatrzymaj reaper (po zakończeniu dzieci nie powinien mieć już co zbierać).
    zombie_thread_stop = 1;
    if (zombie_thread_started)
        (void)pthread_join(zombie_thread, NULL);
